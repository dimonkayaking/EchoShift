...
public class UI
{
    ...
    {
        var healthBg = new Rectangle(10, 10, 200, 20);
        var healthRect = new Rectangle(10, 10, (int)(200 * ((float)health / maxHealth)), 20);
        ...
        var pct = maxHealth <= 0 ? 0 : (int)Math.Round(100.0 * health / maxHealth);
        var pctText = $"{pct}%";
        var size = TinyFont.Measure(pctText, 2);
        var pos = new Vector2(10 + (200 - size.X) / 2f, 10 + (20 - size.Y) / 2f);
        ...
        var echoBg = new Rectangle(10, 40, 200, 20);
        var echoRect = new Rectangle(10, 40, (int)(200 * ((float)echoProgress / 20)), 20);
        ...
        var echoText = $"ЭХО: {echoPoints}  (ТЕЛ. x{teleportsAvailable})";
